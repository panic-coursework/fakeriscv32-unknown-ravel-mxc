...
import org.altk.lab.mxc.recognizer.ParseRecord
import org.antlr.v4.runtime.ParserRuleContext

fun ParseRecord.ast() = AstContext(this).ast(tree)

private class AstContext(val rec: ParseRecord) {
  val ParserRuleContext.ctx: SourceContext
    get() = SourceContext(rec.source, this)
  ...
  fun ast(input: LhsNewContext) =
    NewExpression(input.ctx, ast(input.newTypeId()))
  ...
  fun ast(input: IntegerLiteralContext) =
    IntegerLiteral(input.ctx, parseInt(input.ctx, input))

  fun ast(input: StringLiteralContext) =
    StringLiteral(input.ctx, unescape(input.ctx, input))
